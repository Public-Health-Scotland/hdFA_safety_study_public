...
n_lb <-  df_asm %>% group_by(hdfa_preg) %>% count() %>% mutate(var = "Number of singleton live births") %>% 
  pivot_wider(names_from = hdfa_preg, values_from = n)
cancer_by_exposed <- df_asm %>%group_by(hdfa_preg) %>% 
...
pyar_by_exposed <- df_asm %>% group_by(hdfa_preg) %>%
...
iqr_follows <- df_asm %>% group_by(hdfa_preg) %>% 
  ...
  mutate(median_w_IQR = paste0(sprintf("%0.1f",round_half_up(med_follow_months,1)), 
                               " (", sprintf("%0.1f", round_half_up(QR1,1)), ", ",
                               sprintf("%0.1f", round_half_up(QR3, 1), ")"))) %>%
...
rate_by_exposed <- df_asm %>% group_by(hdfa_preg) %>%
...
pyar_by_exposed <- df_asm %>% group_by(hdfa_preg) %>%
  ...
  pivot_wider(names_from = hdfa_preg, values_from = pyar_all_ages)
rate_by_exposed0_1 <- df_asm %>% group_by(hdfa_preg) %>%
...
names(stats_0_1) <- c("", "", "")
#1-4
n_lb <- df_asm %>% filter(age_end_follow >=1) %>% 
...
pyar_by_exposed <- df_asm %>% group_by(hdfa_preg) %>%
  ...
  mutate(var = "   Person years at risk within this age group") %>% 
  pivot_wider(names_from = hdfa_preg, values_from = pyar_all_ages)
rate_by_exposed1_4 <- df_asm %>% group_by(hdfa_preg) %>%
...
##5-9yrs
n_lb <- df_asm %>% filter(age_end_follow >=5) %>% 
  group_by(hdfa_preg) %>% count() %>%
  pivot_wider(names_from = hdfa_preg, values_from = n)%>%
  mutate(var="   Number of children retained to follow up at the start of this age group")
cancer_by_exposed <- df_asm %>%group_by(hdfa_preg) %>% 
...
pyar_by_exposed <- df_asm %>% group_by(hdfa_preg) %>%
  ...
  mutate(var = "   Person years at risk within this age group") %>% 
  pivot_wider(names_from = hdfa_preg, values_from = pyar_all_ages)
rate_by_exposed_5_9 <- df_asm %>% group_by(hdfa_preg) %>%
...
##10-13yrs
n_lb <- df_asm %>% filter(age_end_follow >=10) %>% 
  group_by(hdfa_preg) %>% count() %>%
  pivot_wider(names_from = hdfa_preg, values_from = n)%>%
  mutate(var="   Number of children retained to follow up at the start of this age group")
cancer_by_exposed <- df_asm%>%group_by(hdfa_preg) %>% 
...
pyar_by_exposed <- df_asm %>% group_by(hdfa_preg) %>%
  ...
  mutate(var = "   Person years at risk within this age group") %>% 
  pivot_wider(names_from = hdfa_preg, values_from = pyar_all_ages)
rate_by_exposed_10_14 <- df_asm %>% group_by(hdfa_preg) %>%
