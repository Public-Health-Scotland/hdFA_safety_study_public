...
df_ind <- df_ind %>% mutate(pyar_end = case_when(death==0 & is.na(TRANSFER_OUT_CODE)~ as.Date("2023-12-31"),
...
n_lb <-  df_ind %>% group_by(hdfa_preg) %>% count() %>% mutate(var = "Number of singleton live births") %>% 
  pivot_wider(names_from = hdfa_preg, values_from = n)
cancer_by_exposed <- df_ind %>%group_by(hdfa_preg) %>% 
...
pyar_by_exposed <- df_ind %>% group_by(hdfa_preg) %>%
...
iqr_follows <- df_ind %>% group_by(hdfa_preg) %>% 
  ...
  mutate(median_w_IQR = paste0(sprintf("%0.1f",round_half_up(med_follow_months,1)), 
                               " (", sprintf("%.1f",round_half_up(QR1,1)), ", ", 
                               sprintf("%.1f",round_half_up(QR3, 1)), ")")) %>%
...
rate_by_exposed <- df_ind %>% group_by(hdfa_preg) %>%
...
##compute which age ban incidence is in
##compute end dates for ate entering each age band
...
pyar_by_exposed <- df_ind %>% group_by(hdfa_preg) %>%
  ...
  pivot_wider(names_from = hdfa_preg, values_from = pyar_all_ages)
rate_by_exposed0_1 <- df_ind %>% group_by(hdfa_preg) %>%
...
names(stats_0_1) <- c("", "", "")
#1-4
n_lb <- df_ind %>% filter(age_end_follow >=1) %>% 
...
pyar_by_exposed <- df_ind %>% group_by(hdfa_preg) %>%
  ...
  mutate(var = "   Person years at risk within this age group") %>% 
  pivot_wider(names_from = hdfa_preg, values_from = pyar_all_ages)
rate_by_exposed1_4 <- df_ind %>% group_by(hdfa_preg) %>%
...
##5-9yrs
n_lb <- df_ind %>% filter(age_end_follow >=5) %>% 
  group_by(hdfa_preg) %>% count() %>%
  pivot_wider(names_from = hdfa_preg, values_from = n)%>%
  mutate(var="   Number of children retained to follow up at the start of this age group")
cancer_by_exposed <- df_ind %>%group_by(hdfa_preg) %>% 
...
pyar_by_exposed <- df_ind %>% group_by(hdfa_preg) %>%
  ...
  mutate(var = "   Person years at risk within this age group") %>% 
  pivot_wider(names_from = hdfa_preg, values_from = pyar_all_ages)
rate_by_exposed_5_9 <- df_ind %>% group_by(hdfa_preg) %>%
...
rates <- rbind(rate_exposed, rate_unexposed) %>%
  mutate(rate_per = paste0(sprintf("%0.1f",round_half_up(est,1)), " (", 
                           sprintf( "%.1f", round_half_up(lower,1)), ", ",
                           sprintf( "%.1f", round_half_up(upper,1)), ")")) %>%
  select(hdfa_preg, rate_per) %>%
  mutate(var= "   Childhood cancer rate within this age group (per 100,000 PYAR) (95% CI)") %>%
  pivot_wider(names_from = hdfa_preg, values_from = rate_per)
...
##10-13yrs
n_lb <- df_ind %>% filter(age_end_follow >=10) %>% 
  group_by(hdfa_preg) %>% count() %>%
  pivot_wider(names_from = hdfa_preg, values_from = n)%>%
  mutate(var="   Number of children retained to follow up at the start of this age group")
cancer_by_exposed <- df_ind%>%group_by(hdfa_preg) %>% 
...
pyar_by_exposed <- df_ind %>% group_by(hdfa_preg) %>%
  ...
  mutate(var = "   Person years at risk within this age group") %>% 
  pivot_wider(names_from = hdfa_preg, values_from = pyar_all_ages)
rate_by_exposed_10_14 <- df_ind %>% group_by(hdfa_preg) %>%
