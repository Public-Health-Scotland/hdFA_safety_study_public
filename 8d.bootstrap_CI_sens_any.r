...
#preprep not in loop#
df<- readRDS(paste0(folder_data_path, "working_data/Sens_Any_dataset_clean_wts.rds"))
df <- df %>%
...
system.time(bootres <- foreach(i=1:B, .combine=rbind) %dopar% {
  ...
  ##refit regression####
  fit_wts <- glm(event ~  time_interval + treated,
  ...
  df_treated$time_interval <- rep(0:12, times=n) 
...
saveRDS(bootres, paste0(folder_data_path, "outputs/sens_any_bootres1_500.rds"))
